...
- **ROC curves** and AUC as tools for evaluating classifier performance across all thresholds
...
Once we have $p(X)$, we classify using a **threshold** $\alpha$:

$$\hat{y} = \begin{cases} 1 & \text{if } p(X) \geq \alpha \\ 0 & \text{if } p(X) < \alpha \end{cases}$$

. . .

In our example ($\alpha = 0.5$ by default):

- **Positive class**: healthy kelp forest ($Y = 1$)
- **Negative class**: degraded kelp forest ($Y = 0$)

---

## {#logistic-function data-menu-title="The logistic function"}

[The logistic function and MLE]{.slide-title}
...
::: {.body-text-m}
- Always returns values **between 0 and 1** — a valid probability
- Produces the characteristic **S-shaped curve**
:::

. . .

**Maximum likelihood estimation (MLE):** find $\hat\beta_0$, $\hat\beta_1$ that make the observed training labels as probable as possible:

$$\mathcal{L}(\beta_0, \beta_1) = \prod_{i:\, y_i = 1} p(x_i) \cdot \prod_{i:\, y_i = 0} \bigl(1 - p(x_i)\bigr)$$

For each positive observation we want $p(x_i)$ close to 1; for each negative we want it close to 0.
...
Blue = healthy sites, red = degraded sites plotted at 0/1. Overlap near the boundary reflects genuine uncertainty in the data.
:::
...
The fitted model has $\hat{\beta}_0 =$ `{python} f"{beta0_s:.3f}"`, $\hat{\beta}_1 =$ `{python} f"{beta1_s:.3f}"`.

1. What is the predicted status of a site with `temp_anomaly` = 0.5°C? Use $\alpha = 0.5$.

2. A new site has `temp_anomaly` = 3°C. What would you expect its predicted class to be, and why?
:::
...
::: {.body-text-s}
1. Log-odds at $X = 0.5$: `{python} f"{beta0_s + beta1_s * 0.5:.3f}"` → $p(0.5) =$ `{python} f"{1/(1+np.exp(-(beta0_s + beta1_s*0.5))):.3f}"`. This is `{python} "'above'" if 1/(1+np.exp(-(beta0_s+beta1_s*0.5)))>=0.5 else "'below'"` the threshold of 0.5 → predicted `{python} "'**healthy**'" if 1/(1+np.exp(-(beta0_s+beta1_s*0.5)))>=0.5 else "'**degraded**'"`.

2. With $\hat\beta_1 < 0$, a high temperature anomaly of 3°C gives a strongly negative log-odds → very small probability → predicted **degraded**.
:::
...
## {#log-odds-slide data-menu-title="Log-odds and the logit"}

[Log-odds (logit)]{.slide-title}

<hr>

Rearranging the logistic function gives the **log-odds**:

$$\log\left(\frac{p(X)}{1 - p(X)}\right) = \beta_0 + \beta_1 X$$

The log-odds is **linear in** $X$. This gives clean coefficient interpretations:

:::: {.columns}

::: {.column width="50%"}
- $\hat\beta_1 > 0$: increasing $X$ → higher log-odds → higher $p(X)$
- $\hat\beta_1 < 0$: increasing $X$ → lower log-odds → lower $p(X)$
:::

::: {.column width="50%"}
More precisely: holding all other predictors constant, a one-unit increase in $X$ **multiplies the odds** by $e^{\hat\beta_1}$ (the **odds ratio**).
:::
...
[Log-odds vs. probability: two scales, same information]{.slide-title}
...
::: {.body-text-s .center-text}
The log-odds scale is linear; the probability scale is the familiar S-curve. Both representations contain identical information.
:::
...
fig, ax = plt.subplots(figsize=(fig_size_x * 0.9, fig_size_y))
ax.plot(temp_range, log_odds_vals, color='steelblue', linewidth=2)
ax.axhline(0, color='gray', linestyle='--', linewidth=1)
ax.set_xlabel('Temp. anomaly (°C)')
ax.set_ylabel('Log-odds')
ax.set_title('Log-odds vs. temp. anomaly')
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
plt.close()
...
## {#checkin-beta-a data-menu-title="Check-in: sign of β₁ (answer)"}
...
A **negative** $\hat\beta_1$ means that, holding all other predictors constant, as temperature anomaly increases (warmer ocean conditions), the log-odds of a healthy site decrease — and the probability of healthy status decreases.

Warmer-than-average temperatures are associated with kelp stress.
...
## {#decision-boundary-value data-menu-title="Decision boundary value"}

[Decision boundary: the threshold on X]{.slide-title}

<hr>

With threshold $\alpha = 0.5$, we predict *healthy* when $p(X) \geq 0.5$, which is equivalent to log-odds $\geq 0$:

$$\hat\beta_0 + \hat\beta_1 X^* = 0 \quad \Rightarrow \quad X^* = -\frac{\hat\beta_0}{\hat\beta_1} = `{python} f"{x_star:.2f}"` \text{ °C}$$

```{python}
#| label: decision-boundary-value
#| echo: false
#| fig-align: center
#| out-width: "80%"
...
ax.scatter(df['temp_anomaly'], df['status_num'],
           c=[col_map[s] for s in df['status']], alpha=0.25, s=15, edgecolors='none')
ax.plot(temp_range, prob_curve, color='black', linewidth=2, label='p(healthy | temp_anomaly)')
ax.axhline(0.5, color='gray', linestyle='--', linewidth=1, label='threshold = 0.5')
ax.axvline(x_star, color='darkorange', linestyle='--', linewidth=1.8,
           label=f'$X^* = {x_star:.2f}$ °C')
ax.set_xlabel('Temp. anomaly (°C)')
ax.set_ylabel('P(status = healthy)')
ax.legend(fontsize=9)
ax.grid(True, alpha=0.3)
...
::: {.body-text-s .center-text}
Sites with `temp_anomaly` < `{python} f"{x_star:.2f}"` °C → predicted *healthy*; sites above → predicted *degraded*.
:::
...
If $\beta_1 = 0$, then $p(X)$ is constant — the predictor has **no effect** on the response.

::: {.body-text-m}
- $H_0$: $\beta_1 = 0$ — no relationship between $X$ and $Y$
- $H_a$: $\beta_1 \neq 0$ — some relationship exists
:::
...
## {#checkin-hypothesis-a data-menu-title="Check-in: hypothesis test (answer)"}
...
The very small $p$-value for `temp_anomaly` provides strong evidence against $H_0$: temperature anomaly is significantly associated with kelp forest status.
...
::: {.center-text .teal-text .body-text-m}
The decision boundary is a **hyperplane** in predictor space —

always a straight line in 2D, unlike the flexible boundary of KNN.
:::
...
[Fitting logistic regression with two predictors]{.slide-title}
...
::: {.body-text-s .center-text}
Shading = predicted probability of healthy. Gray line = decision boundary ($p = 0.5$). The boundary is always a straight line.
:::
