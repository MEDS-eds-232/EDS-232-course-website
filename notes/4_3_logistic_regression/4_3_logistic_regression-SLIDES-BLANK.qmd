...
for t in thresholds:
    ...
    precisions_t.append(precision_score(y_test_num, y_hat, zero_division=1))
...
## {#logistic-intro-blank data-menu-title="Logistic regression"}
...
## {#logistic-example-blank data-menu-title="Logistic regression"}
...
## {#logistic-function-blank data-menu-title="The logistic function"}
...
- Points overlapping near the boundary reflects uncertainty in the data.
...
- **Null hypothesis** $H_0$: there is no relationship between $X$ and $p(X)$, i.e., $\beta_1 = 0$.
- **Alternative hypothesis** $H_a$: there is some relationship between $X$ and $p(X)$, i.e., $\beta_1 \neq 0$.
...
::: {.body-text-s}
- $\hat\beta_j > 0$: increasing $X_j$ $\Rightarrow$ higher log-odds $\Rightarrow$ higher $p(X)$ (holding other predictors fixed)
- $\hat\beta_j < 0$: increasing $X_j$ $\Rightarrow$ lower log-odds $\Rightarrow$ lower $p(X)$ (holding other predictors fixed)

<!-- **Note:** raw coefficients are in different units (°C vs. μmol/L here), so their magnitudes are not directly comparable for assessing "which predictor matters more." -->
:::

---

## {#multiple-logistic-example data-menu-title="Logistic regression"}

[Multiple logistic regression: example data]{.slide-title}

<hr>

In our example:

- **Predictors**: `temp_anomaly`, `nitrate`
- **Response**: `status`:  *healthy* (class 1/positive) or *degraded* (class 0/negative)
...
- If $p(\text{temp_anomaly}, \text{nitrate}) \geq 0.5$, then predict the site is *healthy*.
- If $p(\text{temp_anomaly}, \text{nitrate}) < 0.5$, then predict the site is *degraded*.


---

## {#multi-logistic-prob-1 data-menu-title="Multiple logistic regression: probability gradient"}

[Logistic regression with two predictors]{.slide-title}

<hr>

```{python}
#| label: multi-logistic-explanation-1
#| echo: false
#| fig-align: center
#| out-width: "95%"

fig, ax = plt.subplots(figsize=(fig_size_x * 1.5, fig_size_y + 0.5))
ax.set_position([0.07, 0.14, 0.75, 0.76])
for label, col in col_map.items():
    mask = y_train == label
    ax.scatter(X_train[mask, 0], X_train[mask, 1],
               c=col, s=20, alpha=0.8, label=label, edgecolors='black')
ax.set_xlim(-2, 4)
ax.set_ylim(x2_range[0], x2_range[-1])
ax.set_xlabel('Temp. anomaly (°C)')
ax.set_ylabel('Nitrate (μmol/L)')
ax.set_title('Training data (210 pts) with two predictors')
...
## {#multi-logistic-prob-2 data-menu-title="Multiple logistic regression: probability gradient"}
...
```{python}
#| label: multi-logistic-explanation-2
#| echo: false
#| fig-align: center
#| out-width: "95%"
...
ax.set_xlabel('Temp. anomaly (°C)')
ax.set_ylabel('Nitrate (μmol/L)')
ax.set_title('P(healthy | temp, nitrate) — fitted logistic regression with two predictors')
ax.legend(title='True status', fontsize=9)
ax.grid(True, alpha=0.2)
cbar_ax = fig.add_axes([0.84, 0.14, 0.02, 0.76])
fig.colorbar(cf, cax=cbar_ax, label='P(healthy | temp, nitrate)')
plt.show()
plt.close()
```
...
## {#multi-logistic-prob-3 data-menu-title="Multiple logistic regression: probability gradient"}
...
```{python}
#| label: multi-logistic-explanation-3
#| echo: false
#| fig-align: center
#| out-width: "95%"
...
ax.set_xlabel('Temp. anomaly (°C)')
ax.set_ylabel('Nitrate (μmol/L)')
ax.set_title('We obtain a model for the probability of healthy class: P(healthy | temp, nitrate)')
ax.grid(True, alpha=0.2)
cbar_ax = fig.add_axes([0.84, 0.14, 0.02, 0.76])
fig.colorbar(cf, cax=cbar_ax, label='P(healthy | temp, nitrate)')
plt.show()
...
## {#ulti-logistic-prob-4 data-menu-title="Multiple logistic regression: decision boundary"}
...
```{python}
#| label: multi-logistic-explanation-4
#| echo: false
#| fig-align: center
#| out-width: "95%"
...
ax.set_xlabel('Temp. anomaly (°C)')
ax.set_ylabel('Nitrate (μmol/L)')
ax.set_title('Selecting a threshold creates a decision boundary for classification')
ax.grid(True, alpha=0.3)
cbar_ax = fig.add_axes([0.84, 0.14, 0.02, 0.76])
fig.colorbar(cf, cax=cbar_ax, label='P(healthy | temp, nitrate)')
plt.show()
...
## {#ulti-logistic-prob-5 data-menu-title="Multiple logistic regression: decision boundary"}
...
```{python}
#| label: multi-logistic-explanation-5
#| echo: false
#| fig-align: center
#| out-width: "95%"
...
ax.set_xlabel('Temp. anomaly (°C)')
ax.set_ylabel('Nitrate (μmol/L)')
ax.set_title('This decision boundary creates two regions to classify observations into healthy/degraded')
ax.grid(True, alpha=0.3)
plt.show()
plt.close()
```
...
## {#ulti-logistic-prob-6 data-menu-title="Multiple logistic regression: decision boundary"}
...
```{python}
#| label: multi-logistic-explanation-6
#| echo: false
#| fig-align: center
#| out-width: "95%"
...
ax.set_xlabel('Temp. anomaly (°C)')
ax.set_ylabel('Nitrate (μmol/L)')
ax.set_title('We can now apply the classifier to make predictions (test data shown)')
ax.legend(title='True status', fontsize=9)
ax.grid(True, alpha=0.3)
plt.show()
plt.close()
...
::: {.teal-text .body-text-m}
We fitted two logistic models: one with one predictor `temp_anomaly` and one using `temp_anomaly` and `nitrate` as predictors. 

Both models use `healthy` as the positive class and the default threshold $\alpha = 0.5$. 

How does performance change when we add `nitrate` as a second predictor?
:::
...
## {#threshold-concept-1 data-menu-title="Classification threshold"}
...
## {#threshold-concept-2 data-menu-title="Classification threshold"}
...
## {#checkin-threshold- data-menu-title="Check-in: threshold effect (answers)"}
...
```{python}
#| label: threshold-precision-recall
#| echo: false
#| fig-align: center
#| out-width: "100%"
...
<div class="page-center vertical-center">
<p class="custom-subtitle bottombr">ROC curves and AUC</p>
</div>
---

## {#roc-intro-blank data-menu-title="TPR and FPR"}

[ROC curves]{.slide-title}
...
- **Closer to upper-left**: better discriminating ability. This is what we aim for
...
::: {.column width="55%"}
![By cmglee, MartinThoma – Roc-draft-xkcd-style.svg, CC BY-SA 4.0, [https://commons.wikimedia.org/w/index.php?curid=109730045](https://commons.wikimedia.org/w/index.php?curid=109730045)](Roc_curve.svg.png){width=95%}
...
## {#checkin-auc-a data-menu-title="Check-in: AUC"}
