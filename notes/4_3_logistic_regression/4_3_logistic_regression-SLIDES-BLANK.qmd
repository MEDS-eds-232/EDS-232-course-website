...
ax.set_xlabel('Temp. anomaly (°C)')
ax.set_ylabel('Nitrate (μmol/L)')
ax.set_title('P(healthy) — fitted logistic regression with two predictors')
ax.legend(title='True status', fontsize=9)
ax.grid(True, alpha=0.2)
cbar_ax = fig.add_axes([0.84, 0.14, 0.02, 0.76])
fig.colorbar(cf, cax=cbar_ax, label='P(healthy)')
plt.show()
plt.close()
```
...
ax.set_xlabel('Temp. anomaly (°C)')
ax.set_ylabel('Nitrate (μmol/L)')
ax.set_title('We obtain a model for the probability of healthy class: P(healthy)')
ax.grid(True, alpha=0.2)
cbar_ax = fig.add_axes([0.84, 0.14, 0.02, 0.76])
fig.colorbar(cf, cax=cbar_ax, label='P(healthy)')
plt.show()
...
Gray line = decision boundary ($\alpha = 0.5$).
:::
...
Gray line = decision boundary ($\alpha = 0.5$). Points in the "wrong" region are misclassified.
:::
