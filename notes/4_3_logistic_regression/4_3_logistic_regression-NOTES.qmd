...
grid_binary = (grid_prob >= 0.5).astype(int)
```

```{python}
#| label: multi-logistic-prob-plot
#| echo: false
#| fig-align: center

fig, ax = plt.subplots(figsize=(fig_size_x * 0.9, fig_size_y + 0.5))
cf = ax.contourf(xx1, xx2, grid_prob, levels=50, cmap='Greens', alpha=0.7, vmin=0, vmax=1)
ax.contour(xx1, xx2, grid_prob, levels=[0.5], colors='gray', linewidths=1.5)
ax.set_xlim(-2, 4)
ax.set_xlabel('Temp. anomaly (°C)')
ax.set_ylabel('Nitrate (μmol/L)')
ax.set_title('P(healthy) — logistic regression with two predictors')
ax.grid(True, alpha=0.3)
...
```{python}
#| label: multi-logistic-test-data
#| echo: false
#| fig-align: center

fig, ax = plt.subplots(figsize=(fig_size_x * 0.9, fig_size_y + 0.5))
ax.contourf(xx1, xx2, grid_binary, levels=[-0.5, 0.5, 1.5],
...
ax.set_xlabel('Temp. anomaly (°C)')
ax.set_ylabel('Nitrate (μmol/L)')
ax.set_title('Test data (90 pts) — classification regions at α = 0.5')
...
So the ROC curve always goes from (0,0) to (1,1). Our draft would look something similar to this
