...
:::{.callout-tip}
## Check-in
In our example, suppose we adjust the classification threshold  so that a study site $X$ gets classified as healthy if $p(X) \geq 0.9$. What type of error (false positives or false negatives) would you expect more of? What does this mean in terms of recall and precision?

Now suppose we adjust the classification threshold to $0.2$. What type of error (false positives or false negatives) would you expect more of? What does this mean in terms of recall and precision?
:::
:::{.callout-tip collapse="true"}
## Discussion
*FILL OUT DISCUSSION*
...
1. What would we be missing if we were to only evaluate the model performance at different thersholds only using overall accuracy?
2. You are managing kelp restoration resources and can only intervene at sites you predict as degraded. Would you want a high or low threshold? Which metric does this prioritize?
:::
...
:::{.callout-tip}
## Check-in
For each of the following values of the classification threshold for the multiple logistic regression classifier on our kelp data, we have the following confusion matrices. Use this information to fill out the table below

columns: threshold, TPR, FPR
rows: 1, 0.7, 0.5, 0.3, 0

Use these values to sketch a ROC curve for this classifier. 
