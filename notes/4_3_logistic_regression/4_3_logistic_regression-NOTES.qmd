...
:::{.callout-tip}
## Check-in
1. A new site has `temp_anomaly` = 3°C. Walk through the steps to predict its class using $p(X)$ with threshold $\alpha = 0.5$.
2. If we used `nitrate` as the predictor instead, what would $p(X)$ be modeling?
:::
:::{.callout-tip collapse="true"}
## Discussion
1. Plug $X = 3$ into the fitted logistic function to obtain $p(3) = \frac{e^{\hat{\beta}_0 + \hat{\beta}_1 \cdot 3}}{1 + e^{\hat{\beta}_0 + \hat{\beta}_1 \cdot 3}}$. Because $\hat{\beta}_1 < 0$ and 3°C is a relatively high temperature anomaly, the log-odds will be strongly negative, giving a small probability. Since $p(3) < 0.5$, the site would be predicted as *degraded*.

2. $p(X)$ would model $P(\text{status} = \text{healthy} \mid \text{nitrate})$ — the probability that a site is healthy given its nitrate concentration.
:::
...
:::{.callout-tip}
## Check-in
Using the fitted model above ($\hat{\beta}_0 =$ `{python} f"{beta0_s:.4f}"`, $\hat{\beta}_1 =$ `{python} f"{beta1_s:.4f}"`), what is the predicted status of a site with `temp_anomaly` = 0.5°C? Use the default threshold $\alpha = 0.5$.
:::
:::{.callout-tip collapse="true"}
## Discussion
The log-odds at $X = 0.5$ is $\hat{\beta}_0 + \hat{\beta}_1 \times 0.5 =$ `{python} f"{beta0_s + beta1_s * 0.5:.4f}"`. Converting to probability: $p(0.5) =$ `{python} f"{1 / (1 + np.exp(-(beta0_s + beta1_s * 0.5))):.4f}"`. Since this is `{python} "above" if 1 / (1 + np.exp(-(beta0_s + beta1_s * 0.5))) >= 0.5 else "below"` the threshold of 0.5, the site is predicted as `{python} "'healthy'" if 1 / (1 + np.exp(-(beta0_s + beta1_s * 0.5))) >= 0.5 else "'degraded'"`.
:::


### Interpreting the coefficients

Logistic regression is a classification method that allows us to do both classification and inference. Once we have fitted our model and estimated the coefficients $\hat{\beta}_i$, we can interpret them to understand relations between the predictor and response variables.
...
Notice that the log-odds is exactly linear in $X$ (top panel), but when we transform back to the probability scale we get the familiar S-shape (bottom panel). The two representations contain identical information — they are just different scales.
...
A negative coefficient means that as temperature anomaly increases (warmer ocean conditions), the log-odds of a healthy site decrease, and therefore the probability of healthy status decreases. Warmer-than-average temperatures are associated with kelp stress.
...
Once we have computed the estimate e test this using a **Z-statistic** (the analogue of the t-statistic in linear regression), defined as:
...
:::{.callout-tip collapse="true"}
## Discussion
`temp_anomaly`has a small $p$-values, providing  evidence that this predictor is associated with kelp forest status. Notice, though that this model ignores the existence of a second predictor. 
:::
...
Four our example data, we can now fit a logistic regression using both `temp_anomaly` and `nitrate` as predictors:
...
:::{.callout-tip collapse="true"}
## Discussion
Adding `nitrate` as a second predictor ...
:::
...
Changing the threshold shifts the classification tradeoff between recall and precision:
...
The graph above shows the precision, recall, and overall accuracy for our multiple logistic regression kelp forest status classifier at different classification threshold levels. 
...
1. What would we miss by evaluating model performance using only overall accuracy across thresholds?
2. You manage kelp restoration and can only intervene at sites predicted as degraded. Should the threshold be high or low? Which metric does this prioritize?
3. Around what threshold do precision and recall intersect? What does that point represent?
:::

:::{.callout-tip collapse="true"}
## Discussion
1. Overall accuracy hides the precision–recall tradeoff. A threshold that maximizes accuracy may still have poor recall (missing many degraded sites) or poor precision (flagging many healthy sites). The green dashed line barely changes across a wide range of thresholds, yet precision and recall shift dramatically — accuracy alone would give a misleading picture of model behavior.

2. A **lower threshold** flags more sites as degraded, so fewer truly degraded sites are missed. This prioritizes **recall** — consistent with the reasoning in the previous lesson: the ecological cost of missing a degraded site (false negative) is higher than unnecessarily intervening at a healthy one.

3. The intersection is the threshold at which precision and recall are equal. It represents the point of maximum balance between the two metrics, equivalent to the threshold that maximizes the $F_1$ score.
:::
...
The code below shows the confusion matrix for our multiple logistic regression classifier at five different thresholds. Use the counts to compute TPR and FPR at each threshold, fill in the table, and sketch what the ROC curve would look like.
...
fig, axes = plt.subplots(1, 2, figsize=(fig_size_x * 1.1, fig_size_y * 1.1))
...
For $\alpha = 0$, every site is predicted positive (healthy), so TPR = 1 and FPR = 1 — you can fill those in directly without a confusion matrix.
...
for t in thresholds_roc:
    ...
    rows.append({'Threshold': t, 'TPR': round(tpr_t, 3), 'FPR': round(fpr_t, 3)})

print(pd.DataFrame(rows).to_string(index=False))
```

At $\alpha = 1.0$ nothing is predicted positive → TPR = 0, FPR = 0 (bottom-left corner of the ROC plot). At $\alpha = 0.0$ everything is predicted positive → TPR = 1, FPR = 1 (top-right corner). Intermediate thresholds trace the curve between these extremes: lowering the threshold raises both TPR and FPR, moving up and to the right along the ROC curve. A good classifier hugs the upper-left corner, achieving high TPR while keeping FPR low.
:::

Every ROC curve traces a path from (0, 0) to (1, 1). A **perfect classifier** would pass through the upper-left corner (0, 1): zero false positives with perfect recall. A **random classifier** produces a diagonal line from corner to corner. The annotated points show the TPR/FPR tradeoff at three specific thresholds.
...
Each point on the ROC curve corresponds to one threshold value. Moving along the curve from lower-left to upper-right corresponds to lowering the threshold. 

A curve **closer to the upper-left corner** represents a better classifier: it achieves high TPR while keeping FPR low.

![By cmglee, MartinThoma – Roc-draft-xkcd-style.svg, CC BY-SA 4.0, [https://commons.wikimedia.org/w/index.php?curid=109730045](https://commons.wikimedia.org/w/index.php?curid=109730045)](Roc_curve.svg.png)
