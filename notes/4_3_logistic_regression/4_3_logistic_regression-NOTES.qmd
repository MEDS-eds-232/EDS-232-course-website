...
## Discussion
1. After having the model $p(X)$, we would plug in $X = 3$ to calculate $p(3)$. If $p(3)<0.5$ then we would predict the site is degraded and if $p(3)\geq0.5$ we would predict the site is healthy. 
...
Logistic regression allows us to do both classification and inference. Once we have fitted our model and estimated the coefficients $\hat{\beta}_i$, we can interpret them to understand relations between the predictor and response variables. 
