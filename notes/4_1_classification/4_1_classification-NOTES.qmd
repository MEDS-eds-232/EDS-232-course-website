...
Once we have predicted the class for every point in our test set and compared it agains its true class, we can count the total number of true/false positives and true/false negatives we obtained. We use the notation:
...
The plot below shows training and test error across a range of $K$ values. 
...
Recall  that **model variance** refers to how much our estimate of $\hat{f}$ would change if we trained it on a different data set while **model bias** refers to the error introduced by the assumptions built into the model. 
...
Test error is minimized at an intermediate $K$ that balances these two sources of error. 
...
- **Large $K$**: the boundary becomes very smooth, determined by a large neighborhood. 
...
1. As $K$ increases, the decision boundary becomes more connectedm encompassing larger areas. At $K = 1$ the boundary is highly irregular, closely following individual training points. At $K = 120$ there's no more boundary since everything has been classified as the majority class. 

2. At $K = 1$ the training error is 0 . With a single nearest neighbor, every training point's closest neighbor is itself, so it is always classified correctly. This is the clearest possible sign of overfitting.

3. $K = 1$ is the most overfit: there's a lot of small classiffication regions (fitting noise) and the training error is zero while the test error is noticeably higher. The gap between training and test error is an indication of overfitting!
:::
