...
TSS = np.sum((df['co2_reduction'] - df['co2_reduction'].mean())**2)
RSS_slr = np.sum((df['co2_reduction'] - model_slr.predict(df[['process']]))**2)
RSS_mlr_val = np.sum((df['co2_reduction'] - model_mlr_sk.predict(df[['process', 'efficiency', 'offsets']]))**2)
adj_r2_slr = 1 - (RSS_slr / (n - 2))     / (TSS / (n - 1))
adj_r2_mlr = 1 - (RSS_mlr_val / (n - 4)) / (TSS / (n - 1))
...
# Backward selection p-value tables
X_proc_eff = sm.add_constant(df[['process', 'efficiency']])
res_proc_eff = sm.OLS(df['co2_reduction'], X_proc_eff).fit()
...
## {#mlr-model-1 data-menu-title="The MLR model"}
...
## {#mlr-model-2 data-menu-title="The MLR model"}
...
## {#estimating-coefficients-1 data-menu-title="The MLR model"}
...
## {#regression-plane-1 data-menu-title="The regression plane"}

[From a line to a plane]{.slide-title}
...
# Fit 2-variable model: process + efficiency
X_2var = sm.add_constant(df[['process', 'efficiency']])
res_2var = sm.OLS(df['co2_reduction'], X_2var).fit()
b0_2 = res_2var.params['const']
b1_2 = res_2var.params['process']
b2_2 = res_2var.params['efficiency']

# Regression plane grid
proc_grid = np.linspace(df['process'].min(),    df['process'].max(),    30)
eff_grid  = np.linspace(df['efficiency'].min(), df['efficiency'].max(), 30)
PP, EE = np.meshgrid(proc_grid, eff_grid)
ZZ = b0_2 + b1_2 * PP + b2_2 * EE

# Subsample 60 points for clarity
rng = np.random.default_rng(42)
idx = rng.choice(n, 60, replace=False)
xs  = df['process'].values[idx]
ys  = df['efficiency'].values[idx]
zs  = df['co2_reduction'].values[idx]
z_hat = b0_2 + b1_2 * xs + b2_2 * ys

fig = plt.figure(figsize=(fig_size_x, fig_size_y * 1.4))
ax  = fig.add_subplot(111, projection='3d')

ax.scatter(xs, ys, zs, color='tomato', s=25, zorder=5)

ax.set_xlabel('process ($K)', labelpad=6)
ax.set_ylabel('efficiency ($K)', labelpad=6)
ax.set_zlabel('CO₂ reduction', labelpad=6)
ax.view_init(elev=18, azim=225)
plt.tight_layout()
plt.show()
plt.close()
```
...
## {#regression-plane-2 data-menu-title="The regression plane"}
...
ax.scatter(xs, ys, zs, color='tomato', s=25, zorder=3)
for x, y, z, zh in zip(xs, ys, zs, z_hat):
    ax.plot([x, x], [y, y], [zh, z], color='#444444', linewidth=0.6)
ax.plot_surface(PP, EE, ZZ, alpha=0.6, cmap='Blues', linewidth=0, antialiased=True)
...
## {#mlr-coeffs-1 data-menu-title="The MLR model"}
...
<br>
 ...
 <div style="position: absolute; bottom: 300px; right: 1000px; font-size: 1em;">                                                                               
  ✏️                    
  </div>  
...
## {#hypotheses-mlr-1 data-menu-title="All coefficients test in MLR"}
...
- $H_a$: $\beta_1\neq0 \Rightarrow$ some relationship between response and predictor. 
 ...
 <div style="position: absolute; bottom: 370px; right: 1000px; font-size: 1em;">                                                                               
  ✏️                    
  </div>  
...
## {#f-statistic-1 data-menu-title="The F-statistic"}
...
<div style="position: absolute; bottom: 270px; right: 1000px; font-size: 1em;">                                                                               
  ✏️                    
  </div>  
...
<div class="page-center vertical-center">
<p class="custom-subtitle bottombr">Model fit: R² and adjusted R²</p>
</div>
...
## {#r2-comparison data-menu-title="Check-in: R² SLR vs MLR"}
...
acc_df = pd.DataFrame({
    ...
    'Adj. R²':    [f'{adj_r2_slr:.3f}', f'{adj_r2_mlr:.3f}'],
})
...
## {#backward-checkin data-menu-title="Check-in: backward selection"}
...
step1_df = pd.DataFrame({
    'Predictor': ['process', 'efficiency', 'offsets'],
    'p-value':   [fmt_p(res_mlr.pvalues['process']),
                  fmt_p(res_mlr.pvalues['efficiency']),
                  fmt_p(res_mlr.pvalues['offsets'])]
})
...
step2_df = pd.DataFrame({
    'Predictor': ['process', 'efficiency'],
    'p-value':   [fmt_p(res_proc_eff.pvalues['process']),
                  fmt_p(res_proc_eff.pvalues['efficiency'])]
})
...
## {#backward-checkin-ans data-menu-title="Check-in: backward selection"}
...
::: {.column width="50%"}
**Forward can be useful when:**

- $p$ is large or $p > n$. Backward requires fitting the full model, which cannot be done if $p>n$.
- You expect only a few predictors to matter.

:::

::: {.column width="50%"}
**Backward can be useful when:**

- $p$ is small and $n$ is large enough to fit the full model 
- Worry about missing a predictor that only becomes significant in the presence of others. 
...
- Mixed selection hedges against the weaknesses of both
- Practitioners may run all three and check whether they agree
- If all three disagree, that signals correlated predictors and no single "right" model

::: {.teal-text .body-text-s}
We will later talk about **regularization** (lasso, ridge) over stepwise selection. These methods perform variable selection and coefficient estimation simultaneously and have better statistical properties.
:::
...
There are may be many potential problems when we try to fit a linear model to our data. 

You will be exploring some of the most common ones in homework 2. 
