...
This works well for a **single, already-decided model**, but breaks down as soon as we need to make any modeling decision.
...
- estimate the accuracy metrics on the test set, and
- keep the test set sealed until the very final evaluation.

. . .

**Cross-validation** is the standard tool for doing this:

- splits the training data into temporary training and validation subsets, 
- fits the model on each split, and 
- averages the resulting errors to produce a stable, honest estimate of test performance.
...
The simplest resampling method and building block for cross-validation:
...
If you used a **50/50 split** (110 points for training / 110 for validation), 
...
**The concern would be that the validation error is higher than the test error** the validation error estimates performance of a model trained on only 110 observations, but the final model will be trained on all 210. 
...
The standard **resampling method** in practice. We use the following steps:
...
How we measure $\text{Err}_i$ depends on the type of problem.
...
Larger mistakes are penalized more because errors are squared.
...
$$\text{Err}_i = \frac{1}{n_i} \sum_{j \in \text{fold } i} \mathbf{1}(\hat{y}_j \neq y_j)$$

$\mathbf{1}(\hat{y}_j \neq y_j) = 1$ if observation $j$ was misclassified, 0 otherwise — the fraction of held-out observations assigned the wrong class.

---

## {#other-metrics data-menu-title="Other metrics in CV"}

[Other metrics]{.slide-title}

<hr>

The error rate (or MSE) is not the only metric that can be estimated with CV.

<br>

**Any scalar performance metric can be substituted:**

::: {.body-text-m}
| Problem type | Examples |
|---|---|
| Classification | Accuracy, precision, recall, $F_1$, AUC |
...
The procedure is identical: compute the chosen metric on each held-out fold, then **average across folds**.

In scikit-learn, the `scoring` parameter of `cross_val_score` controls which metric is used.

---

## {#kfold-example data-menu-title="K-fold CV: example"}

[K-fold CV example: kelp data]{.slide-title}

<hr>

5-fold CV for the two-predictor logistic regression, estimating accuracy, precision, and recall:
...
::: {.body-text-s}
Each row is a separate model fit on a different 4-fold subset and evaluated on the held-out fold. The mean across folds is our estimate of test performance.
:::

---

## {#loocv data-menu-title="LOOCV"}

[Leave-one-out CV (LOOCV)]{.slide-title}
...
- Each model trains on $n - 1$ observations → very little bias
- No randomness: there is only one way to hold out each single observation

**Drawback:**

- Must fit the model $n$ times → prohibitively slow for large datasets or complex models
- $k = 5$ or $k = 10$ achieves comparable estimates at a fraction of the cost
:::
...
A **hyperparameter** is any setting chosen by the analyst that **cannot be learned from the training data** during model fitting.
...
|---|---|---|
| K-Nearest Neighbors | $K$ (neighbors) | Decision boundary smoothness |
| Logistic Regression | Threshold $\alpha$ | Precision–recall tradeoff |
:::
...
1. For each candidate value, compute the $k$-fold CV error on the **training set**.
2. Select the value with the lowest CV error.
3. Refit the model using the selected value on the **full training set**.
...
::: {.center-text .teal-text .body-text-m}
The test set never influences the selection — it is only used for the final reported number.
:::
...
::: {.body-text-s .center-text}
The curve traces the bias–variance tradeoff: small $K$ → overfit (high variance); large $K$ → underfit (high bias).
:::
...
::: {.teal-text .body-text-m}
1. What happens to the decision boundary as $K$ increases from 1 to 50? How does this relate to bias and variance?

2. Once $K$ is selected, on what data should the final model be fit?

3. Why is using the test set to select $K$ incorrect?
:::
...
::: {.body-text-s}
1. Boundary gets smoother. Bias ↑, variance ↓. At $K=1$ perfect fit (overfit); at $K=n$ always predicts majority class.

2. On the **full training set** (all 210 obs). CV only informs the choice of $K$.

3. The test set would influence the modeling decision. The reported accuracy would reflect optimization for those observations, not true generalization.
:::
...
![](parameters_workflow.png){width=80% fig-align="center"}

::: {.body-text-s .center-text}
CV governs all decisions made during model development. The test set is used exactly once, at the end.
:::

---

## {#section-comparing data-menu-title="# Comparing models with CV #" background="#047C90"}

<div class="page-center vertical-center">
<p class="custom-subtitle bottombr">Comparing models with CV</p>
</div>
...
Once you select a model with CV, refit it on the full training set and report test-set performance **once**, as the final evaluation.
...
::: {.teal-text .body-text-m}
1. Which model would you select based on this comparison?
...
## {#model-comparison-a data-menu-title="Check-in: model comparison (A)"}
...
::: {.body-text-s}
1. The model with the lowest CV error.

2. Any time we use the test set to inform a decision — choosing a threshold, tuning $K$, or picking a model type — we are optimizing for those specific observations and the test error is no longer honest. CV solves this in all three cases: it estimates how each candidate generalizes using only the training set, so the test set stays untouched until the very final evaluation.
:::
