...
Any performance metric that gives us a number can be substituted:
...
The procedure is identical: compute the chosen metric on each held-out fold, then **average across folds** to get an estimate of the metric on the test set.
...
Each row is a separate model fit on a different 4-fold subset and evaluated on the held-out fold. 

The mean across folds is our estimate of test performance.
...
We decide to tune the hyperparameter $K$ using CV. For each value of $K$ we  estimate the test error rate using 10-fold CV. 
...
::: {.teal-text}
1. Once $K$ is selected, on what data should the final model be fit?
...
2. The test set would influence the modeling decision. The reported accuracy would reflect optimization for those observations, not true generalization.
...
::: {.teal-text}
1. Which model would you select and why? Is there other information you'd like to take into consideration before deciding?
...
::: {.center-text}
CV governs decisions made during model development. 
:::