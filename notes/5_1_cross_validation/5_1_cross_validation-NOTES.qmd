...
The fix is straightforward: **make modeling decisions using only the training data**, and keep the test set sealed until the very final evaluation. Cross-validation (a type of resampling method) is a standard tool for doing this.
...
Models that rely on distances (like KNN) are sensitive to feature scale: a feature with a larger numeric range will dominate the distance calculation. In practice you would standardize features before fitting these models. The critical constraint is that **the scaler must be fit on the training folds only** and then applied to the validation fold. 
