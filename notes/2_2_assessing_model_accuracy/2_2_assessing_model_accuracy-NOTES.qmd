...
We say a model is **overfitting** the data when it has a small training MSE but a large test MSE and a lower test MSE would have been achieved by a simpler model. From the book:
...
ax.set_xlim(0, 100)
ax.set_ylim(-150, 600)
ax.set_title('Linear (lower variance)')
...
ax.set_xlim(0, 100)
ax.set_ylim(-150, 600)
ax.set_title('Degree 15 (higher variance)')
ax.legend(loc='upper left', fontsize=8)
plt.tight_layout()
plt.show()
plt.close()
...
Roughly speaking and from a theoretical perspective, the expected test MSE can be decomposed as

$\text{expected test MSE} = \text{model variance} + \text{model bias} + \text{irreducible error}.$

Though the formal statement and computation of this formula are beyond the scope of the course, intuitively understanding the three components on the right hand side can help us understand important behaviour different models have relative to a variety of training sets.

<!-- In the future, it would be nice to include a box explaining the maths of where this comes from and address formula 2.7. -->

The irreducible error is the variance associated with the error term, $\text{Var}(\epsilon)$. This is a positive value that is independent of the model selection, so the expected test MSE can never be zero: it is bounded below by $\text{Var}(\epsilon)$. 

The model variance and bias are also non-negative, but these depend on the model being selected. So, to minimize the expected test MSE, our goal is to select a model that simultaneously gives *low variance* and *low bias*. 

TO DO:

- decide how to restructure this relative to the example: mention bias in the example as well?
- add one exercise so students get to the bias-variance tradeoff on their own
- add notes for the last paragraph in this section
- read classification setting and make a small personal summary
