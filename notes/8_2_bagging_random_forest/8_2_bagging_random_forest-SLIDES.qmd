...
- **Out-of-bag error estimation**: a free alternative to cross-validation
- **Variable importance**: which predictors matter most across all trees
...
::: {.body-text-m}
A single decision tree suffers from **high variance**: small changes in the training data can lead to very different trees and very different predictions.
:::
...
The more **uncorrelated** the individual predictions are, the more variance reduction we get from averaging.
:::

. . .

**The challenge:** in practice we only have a single training set. **Bagging** is a way around this.
...
<div class="page-center vertical-center">
<p class="custom-subtitle bottombr">Bagging</p>
</div>
...
::: {.body-text-m}
The idea: generate many training sets from the single one we have by **bootstrapping**, fit a model on each, then aggregate predictions.
:::
...
**The bootstrap:** generate a new training set by sampling **with replacement** from the original.

::: {.body-text-m}
- Each bootstrapped sample is the **same size** as the original
- Some observations appear **more than once**
- Some observations are **left out entirely** (~1/3 on average)
:::

---

## {#bootstrap-example-slide data-menu-title="Bootstrap example"}

[A bootstrap example]{.slide-title}
...
## {#bagging-regression data-menu-title="Bagging for regression"}
...
2. Fit a decision tree on each → $B$ estimates $\hat{f}^{*1}(x), \ldots, \hat{f}^{*B}(x)$
3. **Average** the predictions:

$$\hat{f}_{\text{bag}}(x) = \frac{1}{B} \sum_{b=1}^{B} \hat{f}^{*b}(x)$$

```{python}
#| label: bagging-workflow-slide
#| echo: false
#| fig-align: center
#| out-width: "80%"

fig, ax = plt.subplots(figsize=(fig_size_x * 1.4, fig_size_y * 1.05))
ax.set_xlim(0, 10); ax.set_ylim(0, 7); ax.axis('off')

draw_box(ax, 5, 6.2, 2.2, 0.8, 'Training set', facecolor='#e8e8e8')
boot_xs = [1.5, 5, 8.5]
boot_labels = ['Bootstrap 1', 'Bootstrap 2', '... Bootstrap B']
for bx, bl in zip(boot_xs, boot_labels):
    draw_box(ax, bx, 4.4, 2.0, 0.75, bl, facecolor='#d6ecd2')
    arrow(ax, 5, 5.8, bx, 4.78)
tree_labels = ['Tree 1\n$\\hat{f}^{*1}(x)$', 'Tree 2\n$\\hat{f}^{*2}(x)$',
               '...\n$\\hat{f}^{*B}(x)$']
for bx, tl in zip(boot_xs, tree_labels):
    draw_box(ax, bx, 2.7, 2.0, 0.9, tl, facecolor='#fce5cd')
    arrow(ax, bx, 4.03, bx, 3.15)
draw_box(ax, 5, 1.2, 2.6, 0.75,
         'Average predictions\n$\\hat{f}_{\\mathrm{bag}}(x)$',
         facecolor='#ede0f7', fontsize=9)
for bx in boot_xs:
    arrow(ax, bx, 2.25, 5, 1.58)
plt.tight_layout()
plt.show()
plt.close()
```
...
## {#checkin-cls-bagging data-menu-title="Check-in: classification bagging"}
...
::: {.teal-text .body-text-m}
How would you modify the bagging workflow for a **classification** problem? What changes?
:::

. . .

The workflow is identical, but the final aggregation step changes:
...
$$\hat{f}_{\text{bag}}(x) = \text{mode}\left\{\hat{f}^{*1}(x), \ldots, \hat{f}^{*B}(x)\right\}$$
...
::: {.body-text-m}
When fitting each bootstrapped tree, about **1/3 of the training observations are left out** of that bootstrap sample. These are called the **out-of-bag (OOB) observations** for that tree.
:::

. . .

This gives us a way to estimate test error **without a held-out set**:

::: {.body-text-m}
1. For each observation $(x_i, y_i)$, identify all trees for which it was OOB (~$B/3$ trees)
2. Use only those trees to predict $x_i$ (average for regression, majority vote for classification)
3. Repeat for all $n$ training observations → one OOB prediction per observation
...
## {#checkin-oob-q data-menu-title="Check-in: OOB (question)"}
...
## {#checkin-oob-a data-menu-title="Check-in: OOB (answer)"}
