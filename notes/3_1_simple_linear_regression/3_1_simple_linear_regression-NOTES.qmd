...
[📊 Slides](3_1_simple_linear_regression-SLIDES.qmd) / [📊 Blank slides](3_1_simple_linear_regression-SLIDES-BLANK.qmd)
...
1. The entire interval [`{python} f"{ci_b1_low:.3f}"`, `{python} f"{ci_b1_high:.3f}"`] is above 0 so our procedure points to a positive slope: firms that invest more in cleaner production processes achieve greater CO₂ reductions. Each additional \$1K in process spending is associated with a reduction of between `{python} f"{ci_b1_low:.3f}"` and `{python} f"{ci_b1_high:.3f}"` ktCO₂e per year on average.
...
3. We ask the question: *what is the probability that $|\hat{\beta}_1|$ is more than $|t|$ standard deviations away from zero if the true parameter is $\beta_1=0$?* This probability is the $p$-value.

4. There are two options depending on the answer to this question:
...
1. The very small $p$-value for `process` gives strong evidence that $\beta_1 \neq 0$: there is a evidence of a positive relationship between process upgrade investment and CO₂ emissions reductions.
...
3. A coefficient of roughly 0.046 means that, on average, each additional \$1K invested in cleaner production processes is associated with approximately 0.046 ktCO₂e (46 metric tons) of additional CO₂ reduction.
:::
...
1. An $R^2$ of `{python} f"{R2:.3f}"` means that approximately `{python} f"{R2*100:.1f}"`% of the variability in CO₂ reductions across firms is explained by a linear variation in process upgrade spending alone. The remaining ~`{python} f"{(1-R2)*100:.1f}"`% of variability is driven by other factors not included in this model.
...
```{python}
#| label: train-test-slr

X = df[['process']]
y_series = df['co2_reduction']

X_train, X_test, y_train, y_test = train_test_split(
    X, y_series, test_size=0.3, random_state=42
)

model_train = LinearRegression().fit(X_train, y_train)

train_mse = np.mean((y_train - model_train.predict(X_train))**2)
test_mse  = np.mean((y_test  - model_train.predict(X_test))**2)
...
1. A firm wants to use this model to predict CO₂ reductions for new firms not in this dataset. Which metric should they report, training MSE or test MSE? Why?
2. Our SLR has only two parameters. Based on the bias-variance tradeoff from the previous lesson, would you expect this model to overfit?
:::

:::{.callout-tip collapse="true"}
## Discussion

1.  Test MSE. The goal is prediction on new, unseen firms, not on the data the model was already trained on. The training MSE tells us how well the model fits the data it was optimized on, which is always an overly optimistic estimate of real-world performance.
