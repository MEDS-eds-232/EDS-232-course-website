...
plt.suptitle('GreenAbate dataset – CO₂ reduction vs. each reduction spending', y=1.01)
plt.tight_layout()
plt.show()
plt.close()
```
...
For the GreenAbate data, if RSE ≈ 3.26 ktCO₂e, that means our predictions are off by about 3,260 metric tons on average — whether that is acceptable depends on the scale of reductions in the dataset and the firm's targets.
...
The $R^2$ is approximately 0.61. How would you interpret this in plain language?
:::

::: {.callout-tip collapse="true"}
## Discussion

An $R^2$ of 0.61 means that approximately 61% of the variability in CO₂ reductions across firms can be explained by variation in process upgrade spending alone.
:::
