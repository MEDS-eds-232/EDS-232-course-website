...
fig, axes = plt.subplots(1, 2, figsize=(fig_size_x * 1.12, (fig_size_y + 0.5) * 0.8))

axes[0].plot(z, sigmoid, color='steelblue', linewidth=4)
...
axes[1].plot(z, relu, color='tomato', linewidth=4)
...
## {#classification data-menu-title="Depth and representations"}
...
::: {.body-text-s}
For **classification** with $C$ classes, the output layer produces $C$ raw scores $Z_1, \ldots, Z_C$ (one per class), then applies the **[softmax](https://en.wikipedia.org/wiki/Softmax_function)** function to convert them to the probability of belonging to a given class.
:::

```{python}
#| label: classification-diagram-slides
#| echo: false
#| fig-align: center
#| out-width: "68%"

fig, ax = plt.subplots(figsize=(fig_size_x * 1.7, fig_size_y * 1.5))
ax.set_xlim(0, 16)
ax.set_ylim(-0.5, 9.0)
ax.axis('off')

x_in, x_h1, x_h2, x_z, x_out = 1.2, 3.5, 6.0, 8.5, 13.5
in_ys  = np.linspace(1.5, 7.5, 4)   # 4 input nodes
h1_ys  = np.linspace(0.8, 8.2, 5)   # 5 neurons in hidden layer 1
h2_ys  = np.linspace(0.8, 8.2, 5)   # 5 neurons in hidden layer 2
out_ys = np.linspace(2.0, 7.0, 3)   # 3 output classes
r = 0.38
...
# Connections hidden layer 2 → output scores
for h2y in h2_ys:
    for oy in out_ys:
        ax.annotate('', xy=(x_z - r, oy), xytext=(x_h2 + r, h2y),
                    arrowprops=dict(arrowstyle='-', color='#aaaaaa', lw=0.5))
...
box_yc = 4.5
box_h  = 0.9
rect = plt.Rectangle((box_x0, box_yc - box_h/2), box_x1 - box_x0, box_h,
                      linewidth=1.5, edgecolor='#666666', facecolor='#f5f5f5', zorder=2)
ax.add_patch(rect)
ax.text((box_x0 + box_x1) / 2, box_yc, 'Softmax', ha='center', va='center',
        fontsize=8.5, color='#333333', fontstyle='italic')

# Lines from output score nodes → softmax box
for oy in out_ys:
    ax.annotate('', xy=(box_x0, box_yc), xytext=(x_z + r, oy),
                arrowprops=dict(arrowstyle='-', color='#aaaaaa', lw=0.7))

# Lines from softmax box → probability labels (top node = class 1)
for c, oy in enumerate(out_ys):
    idx = len(out_ys) - c
    ax.annotate('', xy=(x_out - 0.1, oy), xytext=(box_x1, box_yc),
                arrowprops=dict(arrowstyle='->', color='#5BAD6F', lw=1.0))
    ax.text(x_out + 0.1, oy, f'$P(Y={idx}\\mid\\mathbf{{X}})$',
...
for i, iy in enumerate(in_ys):
    ...
    ax.text(x_in - 0.6, iy, f'$X_{idx}$', ha='right', va='center', fontsize=10)

# Hidden layer 1 nodes (top = A1^(1))
for k, hy in enumerate(h1_ys):
    idx = len(h1_ys) - k
    ax.add_patch(plt.Circle((x_h1, hy), r, color='#E8A838', zorder=3))
    ax.text(x_h1, hy, f'$A_{idx}^{{(1)}}$', ha='center', va='center', fontsize=7,
            color='white', fontweight='bold')

# Hidden layer 2 nodes (top = A1^(2))
for k, hy in enumerate(h2_ys):
    idx = len(h2_ys) - k
    ax.add_patch(plt.Circle((x_h2, hy), r, color='#E06C3A', zorder=3))
    ax.text(x_h2, hy, f'$A_{idx}^{{(2)}}$', ha='center', va='center', fontsize=7,
            color='white', fontweight='bold')

# Output score nodes (top = Z1)
for c, oy in enumerate(out_ys):
    idx = len(out_ys) - c
    ax.add_patch(plt.Circle((x_z, oy), r, color='#9B59B6', zorder=3))
    ax.text(x_z, oy, f'$Z_{idx}$', ha='center', va='center', fontsize=9,
            color='white', fontweight='bold')

# Layer labels
ax.text(x_in,  -0.3, 'Input\nlayer',    ha='center', va='top', fontsize=9, color='#4A90D9')
ax.text(x_h1,  -0.3, 'Hidden\nlayer 1', ha='center', va='top', fontsize=9, color='#E8A838')
ax.text(x_h2,  -0.3, 'Hidden\nlayer 2', ha='center', va='top', fontsize=9, color='#E06C3A')
ax.text(x_z,   -0.3, 'Output\nscores',  ha='center', va='top', fontsize=9, color='#9B59B6')

ax.set_title('Neural network for classification (C = 3 classes)', fontsize=11, pad=10)
plt.tight_layout()
plt.show()
plt.close()
```
...
![ISLP FIGURE 10.6](tiger-cnn.png)
