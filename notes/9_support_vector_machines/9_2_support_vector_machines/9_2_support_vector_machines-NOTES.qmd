...
We can see in the diagram there is no single cut point on $X$ that can separate the two classes. A single point in one dimension would be a linear boundary.

Now consider adding the feature $X^2$:

| Observation | Class | $X$ | $X^2$ |
|:-----------:|:-----:|------:|--------:|
| 1 | −1 | −3.0 | 9 |
| 2 | −1 | −1.5 | 2.25 |
| 3 | +1 | −0.5 |0.25 |
...
ax.set_xlabel('$X$')
ax.set_ylabel('$X^2$')
ax.set_title('Augmented feature space $(X,\\ X^2)$ — classes are now separable')
...
The support vector classifier found a linear boundary in the $(X, X^2)$ space. Whend we project that boundary  does that boundary look like when projected back onto the original number line?
