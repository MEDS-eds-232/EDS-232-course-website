...
We introduce three widely used kernels below.

### Linear kernel

$$K(x_i, x_j) = \langle x_i, x_j \rangle$$

The linear kernel computes the plain dot product between two observations — it is equivalent to the support vector classifier with no feature augmentation.

**Hyperparameters** (`SVC(kernel='linear', ...)`) :

| Parameter | What it controls |
|:----------|:----------------|
| `C` | Penalty for margin violations — the only parameter to tune (see previous lesson) |
...
The linear kernel cannot capture the circular structure of this data — no straight line can separate an inner cluster from an outer ring.

### Polynomial kernel

$$K(x_i, x_j) = (\gamma \langle x_i, x_j \rangle + r)^d$$

The polynomial kernel captures interactions and powers of the original features — a higher degree allows the boundary to bend and curve more.

**Hyperparameters** (`SVC(kernel='poly', ...)`) :
...
| `coef0` | Controls the influence of lower- vs. higher-degree terms; set to `1` to match the standard textbook formula, `0` for a purely homogeneous polynomial | `0.0` |
| `gamma` | Scaling factor for the dot product; typically left at the default | `'scale'` |

`degree` is the main lever for controlling boundary flexibility. A degree-1 polynomial kernel is identical to the linear SVC; higher degrees produce increasingly complex boundaries.
...
With a degree-3 polynomial kernel the SVM draws a curved boundary that correctly separates the inner cluster from the outer ring.
