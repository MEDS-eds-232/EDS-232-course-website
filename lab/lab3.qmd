...
X = df_model[['E_NPL', 'E_TRI', 'E_TSD', 'E_RMP', 'E_DSLPM', 'E_PM', 'E_POV200', 'E_MINRTY']].values
...
There are two different methods to calculate precision, recall, and F1 when you have multiple classes: the macro average and the micro average. Let's take a look at both.
...
Macro average treats every class equally. **Micro average** pools all TP, FP, and FN counts across every class first, then computes one global metric.

$$\text{Micro Precision} = \frac{\sum_i TP_i}{\sum_i (TP_i + FP_i)} \qquad \text{Micro Recall} = \frac{\sum_i TP_i}{\sum_i (TP_i + FN_i)}$$

Using the per-class TP, FP, and FN values you already computed above:

1. Sum all TP values → `total_TP`, all FP → `total_FP`, all FN → `total_FN`
2. Compute `micro_precision`, `micro_recall`, and `micro_f1`
3. Compare all three to `accuracy_manual` . Do you notice anything interesting abuot these three results? 

```{python}
# Pool TP, FP, FN counts across all three classes
total_TP = TP_low + TP_med + TP_high
total_FP = FP_low + FP_med + FP_high
total_FN = FN_low + FN_med + FN_high

# Compute micro-averaged metrics from the pooled counts
...
print(f"Overall Accuracy: {accuracy_manual:.4f}")
```
...
| Function | Key argument | What it returns |
|---|---|---|
| `accuracy_score(y_test, y_pred)` | — | Overall fraction correct |
| `precision_score(y_test, y_pred, average=...)` | `'macro'` / `'micro'` | Precision averaged across classes |
| `recall_score(y_test, y_pred, average=...)` | `'macro'` / `'micro'` | Recall averaged across classes |
| `f1_score(y_test, y_pred, average=...)` | `'macro'` / `'micro'` | F₁ averaged across classes |

Compute all four metrics for both macro and micro averaging and confirm they match your manual calculations above.
...
sk_micro_precision = precision_score(y_test, y_pred_knn3, average='micro')
sk_micro_recall    = recall_score(y_test, y_pred_knn3, average='micro')
sk_micro_f1        = f1_score(y_test, y_pred_knn3, average='micro')
...
print()
print(f"Micro Precision   : {sk_micro_precision:.4f}")
print(f"Micro Recall      : {sk_micro_recall:.4f}")
print(f"Micro F1          : {sk_micro_f1:.4f}")
```
