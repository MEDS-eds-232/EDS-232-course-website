...
**TensorFlow** (Google) is a framework for building **neural networks and deep learning models**. It is more powerful but more complex than scikit-learn.

| Use scikit-learn when... | Use TensorFlow when... |
|--------------------------|------------------------|
| Dataset is small/medium | You have millions of rows |
| Classical algorithm is sufficient | You need deep neural networks |
| Fast iteration and interpretability | Working with images, audio, or text |

**Keras** is TensorFlow's high-level API, built in as `tf.keras` in TF 2.x. Here is a preview:
...
> **Note:** We won't run TensorFlow in this lab to keep setup simple, but the sklearn patterns you learn — split, scale, train, evaluate — apply in spirit to TF/Keras too.
...
TensorFlow     ->  Deep learning / neural networks (production scale)
```
...
Before diving into data, let's ground ourselves in the theoretical framework from lecture.

---

### The Fundamental Equation

Every statistical learning problem can be written as:
...
| **X** | Predictors / features — our inputs |
| **f** | The true, unknown relationship between X and Y |
| **ε** | Irreducible error — random noise that no model can eliminate |

**Our job:** estimate f as accurately as possible using data.
...
| Goal | Core Question |
|------|---------------|
| **Prediction** | What will Y be for new inputs? |
| **Inference** | Which predictors matter? How? |

**Forest fire example:**

- *Prediction*: Will tomorrow be a fire day? → Use the most accurate classifier.
- *Inference*: Does temperature or humidity drive fire risk more? → Use an interpretable model where coefficients have clear meaning.
